...
export async function POST(request: Request) {
  try {
    ...
    const response = await safePropertyPortalFetch(fetch, buildPropertyPortalUrl("/api/broker/intake"), {
      method: "POST",
      body: formData,
    }, "new listing intake media upload");
    ...
    const normalized = createPropertyPortalProxyError(error, "new listing intake media upload");
