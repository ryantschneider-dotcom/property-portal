...
export async function POST(request: Request) {
  try {
    ...
    const response = await safePropertyPortalFetch(fetch, buildPropertyPortalUrl("/api/admin/properties/auto-enrich"), {
      method: "POST",
      cache: "no-store",
      headers: {
    ...
    const normalized = createPropertyPortalProxyError(error, "Auto-Enrich Data");
