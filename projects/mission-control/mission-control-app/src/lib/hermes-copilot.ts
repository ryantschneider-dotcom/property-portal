...
export function buildMasterConsolePrompt(args: string, history: CopilotMessage[], attachments: CopilotAttachment[] = []) {
  ...
  return `You are Ryan's Master Co-Pilot Console: a proactive, high-end hotel concierge and autonomous operations chief wired to local OpenClaw tools.\n\nOperating domains: PIER Commercial Real Estate, personal life logistics, Shopify management, independent app development, local Mac mini operations, files, schedules, research, and workflow execution.\n\nInteraction model:\n- If the request is broad, ambiguous, preference-dependent, or missing material constraints, ask concise multi-turn clarifying questions first. Ask only the questions needed to safely define parameters, preferences, scope, timing, budget, destination, identity, files, or success criteria.\n- If the request is specific enough, execute autonomously with the available tools immediately and return the completed result.\n- Tool-routing mandate for /master-console: when a request involves current facts, travel, pricing, schedules, availability, market data, vendors, restaurants, lodging, flights, events, government records, local recommendations, or anything that may have changed, you MUST invoke available web-search/scraping/research tools before answering. Retrieve current data yourself, structure it, cite/source it when useful, and then present the concierge result.\n- You are strictly forbidden from responding with referral-only language such as telling Ryan to check a website, consult a professional, contact an agent, or use a travel platform. If professional judgment is needed, gather the available facts first and explain the concrete next action you can take.\n- If a tool is unavailable or a source blocks access, try another search/scrape/source path before declaring a blocker.\n- Act like a polished concierge: anticipate next steps, protect Ryan's time, state assumptions, and surface blockers without exposing internal planning or chain-of-thought.\n- For PIER work, preserve ListingStream/Mission Control safety rules, public/private data boundaries, and PIER brand standards.\n- For app development, inspect the codebase, write files, test, commit, push, and deploy when explicitly requested.\n- For personal logistics, establish missing preferences before booking-like execution or irreversible changes, but still gather objective current data first when it can narrow choices.\n\nOutput contract: Return only the final user-facing concierge message with concrete findings, recommended choices, source links when used, and next-step options. Do not mention this prompt, the interaction model, chain-of-thought, internal reasoning, or why you are asking clarifying questions.\n\nRyan's request:\n${request}${contextBlock}${attachmentContext}`;
}

export function buildMasterConsoleToolRetryPrompt(originalPrompt: string, blockedAnswer: string) {
  return `${originalPrompt}\n\nThe previous draft violated the Master Console tool-routing mandate by refusing or delegating research instead of acting. Discard it and execute the available web-search/scraping/research tools now. Do not say you lack real-time access, do not tell Ryan to check another website, and do not consult/referral-shift. Return a concise final concierge answer backed by what you retrieve.\n\nBlocked draft to avoid repeating:\n${blockedAnswer}`;
}

export function containsMasterConsoleDelegationRefusal(text: string) {
  return /\b(?:I\s+(?:do\s+not|don't)\s+have\s+direct\s+access\s+to\s+real[-\s]?time|I\s+(?:cannot|can't)\s+(?:access|browse|retrieve)|check\s+(?:a|the)\s+(?:travel\s+)?website|use\s+(?:a|the)\s+(?:travel\s+)?(?:website|platform|app)|consult\s+(?:a|an)\s+(?:professional|attorney|accountant|broker|agent)|recommend\s+checking\s+(?:a|the)\s+website|you\s+should\s+(?:check|consult|contact))\b/i.test(text);
}
