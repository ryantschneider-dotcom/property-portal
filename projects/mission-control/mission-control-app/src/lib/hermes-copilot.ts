...
export const MASTER_CONSOLE_DURABLE_MEMORY_CONTEXT = `Durable Hermes memory profile for this console:
- You are speaking with Ryan T. Schneider, CCIM, Founder/Principal and President of PIER Commercial Real Estate Brokerage and Management.
- Ryan is based around Isle of Hope / Savannah, Georgia; default personal travel and local logistics context starts from Savannah, GA unless Ryan says otherwise.
- For flights/travel, assume Savannah/Hilton Head International (SAV) is the baseline departure airport and consider CHS/JAX/ATL as alternates only when helpful; do not ask for Ryan's departure city unless the request explicitly changes origin.
- Ryan's PIER contact block: Ryan T. Schneider, CCIM, President, ryan@piercommercial.com, 912-239-6298.
- Ryan's CRE work centers on property management, leasing, and development around Savannah/Coastal Georgia, including Whitemarsh Plaza, the Fred Williams Building, the Lindsay and Morgan Building, and 2812 Williams Street.
- PIER brand defaults: #CB521E orange with gray, official PIER public-facing outputs only, and strict public/private data separation.
- Tenant aliases: defy, ijump, and skyzone can refer to the same entity in Ryan's lease summaries/documentation.
- Ryan prefers autonomous, data-safe execution: inspect files, test/build/deploy when requested, verify live output, and avoid unrelated uncommitted files.`;

function formatCopilotAttachmentContext(attachments: CopilotAttachment[]) {
  if (!attachments.length) return "";
  const lines = attachments.map((attachment, index) => {
    const type = attachment.contentType.toLowerCase();
    const imageHint = type.startsWith("image/") ? " Image/screenshot attachment; inspect visual details from this URL when answering." : "";
    const videoHint = type.startsWith("video/") ? " Video/media attachment; use available media tools or storage URL instead of asking Ryan to re-describe it." : "";
    const documentHint = /pdf|word|document|text|csv|json|spreadsheet|sheet|presentation|powerpoint/.test(type) ? " Document attachment; read parsed document context below first, then use storage/URL tools for deeper extraction if needed." : "";
    const storageHint = attachment.storagePath ? ` Storage path: ${attachment.storagePath}.` : "";
    return `${index + 1}. ${attachment.name} (${attachment.contentType}, ${attachment.size} bytes): ${attachment.url}.${storageHint}${imageHint}${videoHint}${documentHint}`;
  });
  return `\n\nAttached Mission Control files:\n${lines.join("\n")}\nUse these active URLs/data as first-class context. If an attachment is an image, screenshot, PDF, lease, video, or document, inspect or use the parsed media context before asking Ryan to describe it manually.`;
}
