...
- Documents, attachments, and links are mutable arrays. When broker feedback says to remove/delete/drop/hide/unpublish/take down a document, attachment, file, URL, or link, semantically match against current documents/attachments/links objects by id, title, name, label, description, filename, url, href, or downloadUrl, then return the complete resulting array with only that object removed. A removal can only be treated as successful when the output array length is strictly less than the input array length; otherwise flag it and do not claim success.
