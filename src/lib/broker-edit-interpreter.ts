...
function extractTitle(instructions: string) {
  const quoted = instructions.match(/(?:listing\s+title|title)\s*(?:to|as|=|:)?\s*["“]([^"”\n]{4,140})["”]/i);
  if (quoted) return quoted[1].trim();
  const plain = instructions.match(/(?:listing\s+title|title)\s*(?:to|as|=|:)\s*([^\n\.]{4,140})/i);
...
export function interpretBrokerEditRequest(rawProperty: Record<string, unknown>, instructions: string): BrokerEditInterpreterResult {
  ...
  const property = asRecord(rawProperty.property);
  ...
  if (leasedSuite) {
    ...
    if (!suites.length) {
      ...
      const remainingSuites = suites.filter((suite) => suite.suiteNumber.toLowerCase() !== leasedSuite.toLowerCase());
  ...
  if (Object.keys(nextContent).length) updatePayload.content = nextContent;
  if (Object.keys(nextAdmin).length) updatePayload.admin = nextAdmin;

  const confidence: BrokerEditInterpreterResult["confidence"] = summary.length >= 2 && flags.length === 0
    ? "high"
    : summary.length >= 1
      ? "medium"
      : "low";
