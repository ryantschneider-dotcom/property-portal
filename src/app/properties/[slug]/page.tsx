  ...
  const facts = [
    {
      ...
      value: property.property.buildingSizeSf ? property.property.buildingSizeSf.toLocaleString() : "—",
    },
    {
      label: "Acres",
      value: property.property.lotSizeAcres ? String(property.property.lotSizeAcres) : "—",
    },
    {
      label: "Year Built",
      value: property.property.yearBuilt ? String(property.property.yearBuilt) : "—",
    },
    {
      label: "Zoning",
      value: property.property.zoning ?? "—",
    },
    {
      label: "Parcel ID",
      value: property.property.parcelId ?? "—",
    },
    {
      label: "Property Type",
      value: property.property.category ?? "—",
    },
  ];
  ...
  return (
    <main className="min-h-screen bg-zinc-50 text-zinc-900">
      <section className="mx-auto flex max-w-7xl flex-col gap-8 px-6 py-10 lg:py-14">
        <div>
          <Link href="/" className="text-sm font-medium text-zinc-600 transition hover:text-zinc-900">
            ← Back to listings
          </Link>
        </div>
          ...
          <div className="space-y-8 lg:sticky lg:top-6">
            <section className="rounded-[2rem] border border-zinc-200 bg-white p-8 shadow-sm">
              ...
              <p className="mt-3 text-sm leading-6 text-zinc-600">
                Premium detail panel for pricing, quick facts, and direct access to the custom property website.
              </p>

              <dl className="mt-6 space-y-3 text-sm">
                <div>
                  <dt className="text-zinc-500">Sale Price</dt>
                  <dd className="font-medium text-zinc-900">{property.pricing.salePriceDollars?.toLocaleString() ?? "—"}</dd>
                </div>
                <div>
                  <dt className="text-zinc-500">Price / Unit</dt>
                  <dd className="font-medium text-zinc-900">{property.pricing.salePricePerUnit ?? "—"}</dd>
                </div>
                <div>
                  <dt className="text-zinc-500">Pricing Units</dt>
                  <dd className="font-medium text-zinc-900">{property.pricing.salePriceUnits ?? "—"}</dd>
                </div>
                <div>
                  <dt className="text-zinc-500">Parcel ID</dt>
                  <dd className="font-medium text-zinc-900">{property.property.parcelId ?? "—"}</dd>
                </div>
                <div>
                  <dt className="text-zinc-500">Zoning</dt>
                  <dd className="font-medium text-zinc-900">{property.property.zoning ?? "—"}</dd>
                </div>
              </dl>

              {property.links.websiteUrl ? (
                <div className="mt-6">
                  <PropertyWebsiteLink url={property.links.websiteUrl} />
                </div>
              ) : null}
            </section>
