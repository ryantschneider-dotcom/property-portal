...
function asText(value: unknown, fallback = ""): string {
  ...
  if (typeof value === "object") {
    ...
    for (const key of ["full", "street", "name", "label", "value", "title", "city"]) {
        ...
        if (docSnap.exists()) {
          ...
          setFormData((prev) => ({
            ...
            visibility: {
              ...
              transactionLabel: asText(data.visibility?.transactionLabel, prev.visibility.transactionLabel),
            },
            property: {
              ...prev.property,
              ...(typeof data.property === "object" && data.property ? data.property : {}),
              category: asText(data.property?.category, prev.property.category),
              buildingSizeSf: asNumberInput(data.property?.buildingSizeSf),
              lotSizeAcres: asNumberInput(data.property?.lotSizeAcres),
              yearBuilt: asNumberInput(data.property?.yearBuilt),
            },
            address: {
              ...prev.address,
              ...(typeof data.address === "object" && data.address ? data.address : {}),
              full: asText(data.address?.full),
              street: asText(data.address?.street),
              city: asText(data.address?.city),
              state: asText(data.address?.state),
              zip: asText(data.address?.zip),
            },
            pricing: {
              ...prev.pricing,
              ...(typeof data.pricing === "object" && data.pricing ? data.pricing : {}),
              salePriceDollars: asNumberInput(data.pricing?.salePriceDollars),
            },
            content: {
              ...prev.content,
              ...(typeof data.content === "object" && data.content ? data.content : {}),
              saleTitle: asText(data.content?.saleTitle),
              saleDescription: asText(data.content?.saleDescription),
              locationDescription: asText(data.content?.locationDescription),
            },
          }));
        }
      } catch (error) {
    ...
    try {
      ...
      const payload = {
        ...
        property: {
          ...
          yearBuilt: Number(formData.property.yearBuilt) || null,
        },
      };
    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Editing: {asText(formData.title, "Untitled")}</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="col-span-full">
              <label className="block text-sm font-bold text-gray-700 mb-2">
                Property Title (Internal & Display)
              </label>
              <input
                type="text"
