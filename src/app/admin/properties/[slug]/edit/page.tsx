...
export default async function EditAdminPropertyPage({
  ...
  if (session.role !== "admin" && workflow?.ownerEmail?.toLowerCase() !== session.email.toLowerCase()) {
    notFound();
  }
  ...
  return (
    <main className="px-6 py-10 lg:px-10 lg:py-12">
      <div className="mb-8">
        <p className="text-sm font-medium uppercase tracking-[0.2em] text-zinc-500">{session.role === "admin" ? "Admin Dashboard" : "Broker Draft Review"}</p>
